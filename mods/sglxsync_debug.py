...
## Function added for the case that the neuropixel came unplugged during the recording
##   in this case, ref_stream (ap_0) will be shorter than nidq and wav streams, 
##   which will be truncated to match its length     -- Lo
def sync_all_unplugged_npx(all_syn_dict: dict, ref_stream: str, force=False) -> dict:
    logger.info('syncing all times to {}'.format(ref_stream))
    ref_syn_dict = all_syn_dict[ref_stream]
    for one_stream, one_syn_dict in all_syn_dict.items():
        if one_stream==ref_stream:
        ...
        if not(os.path.exists(t_p_path) and (force is False)):
            ...
            # Edit to truncate one_stream to length of ref_stream:
            len_ref = np.shape(all_syn_dict[ref_stream]['t_0'])[0]/all_syn_dict[ref_stream]['s_f'] # length of ref stream (s)
            end_ref = int(len_ref * all_syn_dict[one_stream]['s_f']) # where to truncate onestream (samples)
            len_ref_evt_arr = np.shape(all_syn_dict[ref_stream]['evt_arr'])[1] # number of events captured in ref stream
            
            t_prime = su.sync_to_pattern(one_syn_dict['evt_arr'][:,:len_ref_evt_arr], one_syn_dict['t_0'][:end_ref],
                                                     ref_syn_dict['evt_arr'], ref_syn_dict['t_0'])
            logger.info('    saving t_prime array to ' + t_p_path)
            np.save(t_p_path, t_prime)
...
## Function added for the case that the neuropixel came unplugged during the recording
##   all bouts recorded after the neuropixel came unplugged will be discarded     -- Lo
def bout_dict_from_pd_unplugged_npx(bout_pd: pd.DataFrame, all_syn_dict: dict, s_f_key: str='wav') -> dict:
    s_f = all_syn_dict[s_f_key]['s_f']
