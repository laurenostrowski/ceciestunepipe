...
def extract_imec_syn_ch(raw_data: np.array, first_samp:int, last_samp:int, meta_dict:dict) -> np.array:
    ## extract the 
        ...
        if n_sy == 0:
            ...
            i_dig_ch = n_ap + n_lf
            # Imec probes read out 384 channels of neural data and have 8 bits of status data (stored as a 16-bit SY word)
            # bit #6 is the sync waveform, the other bits are error flags
            ...
            if np.any(error_arr):
                ...
                logger.info('There were {} errors in the recording'.format(n_errors))
        return dig_array
