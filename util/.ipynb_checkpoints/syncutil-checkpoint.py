...
def quick_ttl_threshold(x:np.array) -> float:
...
def sync_to_pattern(x_ttl: np.array, t: np.array, x_0_ttl: np.array, t_0:np.array) -> np.array:
    ...
    with open('/mnt/cube/lo/x_ttl.pickle','wb') as handle:
        pickle.dump(x_ttl, handle)
    with open('/mnt/cube/lo/t.pickle','wb') as handle:
        pickle.dump(t, handle)
    with open('/mnt/cube/lo/x_0_ttl.pickle','wb') as handle:
        pickle.dump(x_0_ttl, handle)
    with open('/mnt/cube/lo/t_0.pickle','wb') as handle:
